...
export const parseDmnContent = ({
  ...
  return excelSheet.map((sheet, idx) => {
    ...
    return dmnContents({
      ...
      rules: getRules(safeRuleRows, amountOutputs, header.length, hasAnnotationColumn, addQuotes)
    });
  });
...
const getRules = (rows = [], amountOutputs, headerLength, hasAnnotationColumn, addQuotes) => {
  return rows.map((row) => {
    const ruleData = { id: nextId('Rule_'),
      description: hasAnnotationColumn ? row[row.length - 1] : '',
      inputEntries: getEntries(row.slice(0, headerLength - amountOutputs), 'InputEntry', addQuotes),
      outputEntries: getEntries(row.slice(headerLength - amountOutputs, headerLength), 'OutputEntry', addQuotes)
    };
...
const getEntries = (row = [], rowType, addQuotes) => {
  return row.map((text) => entry(nextId(`${rowType}_`), getType(text) == 'string' && addQuotes && text.length != '' ? '"' + text + '"' : text));
};
